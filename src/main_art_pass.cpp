...
// The classifieds deliberately rotate through the upstream pack's five civilian
// colour families to look like an actual mixed used-car page instead of clones.
//
...
inline const EmbeddedSprite& classifieds_car(int cx, int base_y) {
    const unsigned slot = static_cast<unsigned>((cx / 90 + base_y / 70) % 5);
    switch (slot) {
        case 0: return kenney_iso_red_sedan;
        case 1: return kenney_iso_blue_sedan;
        case 2: return kenney_iso_green_sedan;
        case 3: return kenney_iso_silver_sedan;
        default: return kenney_iso_black_sedan;
    }
}
